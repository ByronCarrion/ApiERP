    ...
    public class RequisaInsertHeadToMCS
    {
        ...
        public bool InsertaRequisaMCS()
        {
            try
            {
                ...
                dt = con.ExecuteTable($"INSERT INTO dbo.tRequisas (Fecha,CodBodega,Uso,Departamento,EntregarA,CodSeccion,CtaContable,CentroCosto,iActivityOT,Finca,Lote,Equipo,Actividad,InternalCode,Estado,CreadoPor,CreadoEl) " +
                    $"VALUES('{Fecha}','{CodBodega}','{Uso}','{Departamento}','{EntregarA}','{CodSeccion}','{CtaContable}','{CentroCosto}','{iActivityOT}','{Finca}','{Lote}','{Equipo}','{Actividad}','{InternalCode}','{Estado}','{CreadoPor}',getdate());");
                dt.Clear();
                //VERIFICAMOS EL ULTIMO ID DE REQUISA INSERTADO
                dt = con.ExecuteTable($"SELECT TOP (1) IdRequisa FROM dbo.tRequisas ORDER BY IdRequisa DESC;");
                if (dt.Rows.Count > 0)
